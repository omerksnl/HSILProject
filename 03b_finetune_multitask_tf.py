...
def main():
    parser = argparse.ArgumentParser(description="Stage-2 finetune: multitask on first 300 labels.")
    ...
    parser.add_argument("--pretrain-model", type=str, required=True)
    parser.add_argument("--pretrain-norm-json", type=str, required=True)
    ...
    parser.add_argument("--out-dir", type=str, default="runs/finetune_multitask_from_pretrain")
    ...
    norm = json.loads(Path(args.pretrain_norm_json).resolve().read_text(encoding="utf-8"))
    ...
    model = tf.keras.models.load_model(Path(args.pretrain_model).resolve())
    compile_model(model, args.lr, args.class_loss_w, args.cobb_loss_w)

    callbacks = [
        tf.keras.callbacks.EarlyStopping(
            monitor="val_class_output_accuracy", mode="max", patience=4, restore_best_weights=True
        ),
    ...
    print(f"Finetune val class acc: {eval_res.get('class_output_accuracy', 0.0):.4f}")
    print(f"Finetune val cobb mae(norm): {eval_res.get('cobb_output_mae', 0.0):.4f}")
    ...
    with (out_dir / "finetune_summary.txt").open("w", encoding="utf-8") as f:
        f.write(f"val_class_acc={eval_res.get('class_output_accuracy', 0.0):.4f}\n")
